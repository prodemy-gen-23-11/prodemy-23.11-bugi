...
import { fillFromApi, createNewCartAction, setUserIdStateAction } from "../store/reducers/cartSlice";
...
function Navbar() {
    ...
    const fetcher = (url) =>
        axios
            .get(url)
            .then((response) => {
                dispatch(setUserIdStateAction(user.id))
                dispatch(fillFromApi(response.data.productList));
            })
            .catch(() => {
                if (isLoggedIn)
                    dispatch(createNewCartAction({ userId: user.id }));
            });

    const cartItems = useSWR(`http://localhost:3000/cart/${user.id}`, fetcher, {
    ...
    useEffect(() => setAmountCartItem(cartData?.length));
                            ...
                            <div className="relative flex flex-row items-center mr-2 group cursor-pointer">
                                {amountCartItem > 0 ? (
                                    <div className="absolute w-2.5 h-2.5 text-center rounded-full bg-red-400 top-0 left-4 group-hover:scale-110 duration-300 z-20">
                                        <p className="text-[0.5rem] text-white top-1 group-hover:scale-110 duration-300 z-30">{amountCartItem}</p>
                                    </div>
                                ) : null}
