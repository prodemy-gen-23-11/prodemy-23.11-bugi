...
function DetailDesc({ product }) {
    ...
    const onClickBuyNow = () => {
        //setCartData({ productName, productPrice, productDiscount, quantity });
        const payload = {
            ...product,
            quantity
        }
    ...
    return (
        <div id="desc-container"
...

            {
                product.discount != null && product.discount != 0 ? (
                    <div className="flex flex-row items-baseline gap-2">
                        <h2 className="font-bold text-2xl">{toRupiah(product.price * ((100 - product.discount) / 100))}</h2>
                        <div className="px-2 h-6 bg-sky-400 rounded-lg">
                            <h2 className="font-bold text-zinc-50">{product.discount}% OFF</h2>
                        </div>
                    </div>
                ) : (
                    <h2 className="font-bold text-2xl">{toRupiah(product.price)}</h2>
                )
            }

            <h3 className="text-lg line-through">{toRupiah(product.price)}</h3>
            ...
            <div className="flex flex-row justify-center mb-4 md:justify-between">
                ...
                <button onClick={onClickAddToCart} className="group button bg-sky-400 ml-1 hover:bg-sky-500 hover:scale-110 duration-300 cursor-pointer">
